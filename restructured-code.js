...
function setupEventListeners() {
    ...
    if (gridSizeSlider && gridSizeValue) {
        gridSizeSlider.value = config.gridSize;
        gridSizeValue.textContent = config.gridSize;
    ...
    if (sizeSlider && sizeValue) {
        ...
        sizeSlider.addEventListener('input', function() {
            config.size = parseFloat(this.value);
            sizeValue.textContent = config.size.toFixed(1);
            meshManager.createGridMesh(config.gridSize, config.size);
            meshManager.rebuildGeometry();
        });
    ...
    if (thresholdSlider && thresholdValue) {
        thresholdSlider.value = config.collisionThreshold;
        thresholdValue.textContent = config.collisionThreshold.toFixed(2);
        
        thresholdSlider.addEventListener('input', function() {
            config.collisionThreshold = parseFloat(this.value);
            thresholdValue.textContent = config.collisionThreshold.toFixed(2);
            meshManager.setCollisionThreshold(config.collisionThreshold);
        });
    }
    ...
    if (wireframeToggle) {
        wireframeToggle.checked = config.wireframe;
        
        wireframeToggle.addEventListener('change', function() {
            config.wireframe = this.checked;
            meshManager.toggleWireframe();
        });
    }
